<template>
  <div>
    ...
    >
      ...
      <tbody>
        <tr v-for="container in containers" :key="container.id">
          ...
          <td>
            <b-button
              v-if="container.status == 'Stopped' || container.status == 'Created'"
              v-on:click.prevent="startContainer(container)"
            >Start Container</b-button>
            <b-button
              v-if="container.status == 'Running'"
              v-on:click.prevent="stopContainer(container)"
            >Stop Container</b-button>
            <b-button
              variant="outline-danger"
              v-if="container.status == 'Stopped' || container.status == 'Created' ||  container.status == 'Error'"
    ...
    <createContainer
      @reload-containers="reloadContainers"
      v-if="this.$store.state.selectedTab == 'createContainer'"
      :images="images"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    startContainer(container) {
      ...
      startContainer
        .post("/container/v1/containers/" + container.uuid + "/start")
        .then(response => {
          console.log(response);
    ...
    stopContainer(container) {
      ...
      stopContainer
        .post("/container/v1/containers/" + container.uuid + "/stop")
        .then(response => {
          console.log(response);
          this.reloadContainers();
        })
    ...
    deleteContainer(container) {
      ...
      deleteContainer
        .delete("/container/v1/containers/" + container.uuid)
        .then(response => {
          console.log(response);
          this.reloadContainers();
        })
